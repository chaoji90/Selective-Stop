...
def main(args):
    ...
    def log_result(genotypes,epoch,logging):
        if criterion_one_2_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion one stop 2 skip-connects met')
            logging.info('{:}'.format(genotypes[criterion_one_2_epoch]))
        if criterion_one_3_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion one stop 3 skip-connects met')
            logging.info('{:}'.format(genotypes[criterion_one_3_epoch]))
        if criterion_one_4_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion one stop 4 skip-connects met')
            logging.info('{:}'.format(genotypes[criterion_one_4_epoch]))
        if criterion_two_2_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion two stop for 2 epochs retain')
            logging.info('{:}'.format(genotypes[criterion_two_2_epoch]))
        if criterion_two_3_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion two stop for 3 epochs retain')
            logging.info('{:}'.format(genotypes[criterion_two_3_epoch]))
        if criterion_two_4_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion two stop for 4 epochs retain')
            logging.info('{:}'.format(genotypes[criterion_two_4_epoch]))
        if criterion_two_10_flag:
            logging.info('-'*100)
            logging.info('DARTS+ criterion two stop for 10 epochs retain')
            logging.info('{:}'.format(genotypes[criterion_two_10_epoch]))

        logging.info('-'*100)
        logging.info(f"Selective stop peak epoch: {np.stack(op_mag_list).argmax(axis=0)[stop_op]}")
        logging.info('{:}'.format(genotypes[np.stack(op_mag_list).argmax(axis=0)[stop_op]]))
        logging.info('-'*100)
        logging.info(f"Selective stop res_o epoch: {op_res_list_o.index(max(op_res_list_o))}")
        logging.info('{:}'.format(genotypes[op_res_list_o.index(max(op_res_list_o))]))
    ...
    for epoch in range(start_epochs, args.run.epochs):
        ...
        scheduler.step()
        with torch.no_grad():
            #print(architect.alphas)
            temp=architect.alphas['normal'].cpu().data.numpy()
            softmax_sum=architect.alphas['normal'].cpu().data.numpy().sum(axis=0)
            num_skip=(temp.argmax(axis=1)==1).sum()
            op_res=(softmax_sum[stop_op]-softmax_sum).sum()
            if softmax_sum.argmax()==stop_op: 
            ...
            if not criterion_one_4_flag or not criterion_one_2_flag or not criterion_one_3_flag:
                logging.info(f"Num of skip-connect:{num_skip}")
            ...
            if not criterion_two_10_flag:
                if epoch==0: rank_before=temp.argsort(axis=1)
                    ...
                    if (rank_current==rank_before).all():
                        rank_retain+=1
                        logging.info(f'rank_retain:{rank_retain}')
                        #logging.info(temp.argsort(axis=1))
                    else:
                        rank_retain=1
            ...
            #if criterion_one_4_flag and criterion_two_2_flag and criterion_two_3_flag and criterion_two_4_flag: 
                #log_result(genotypes,epoch,logging)
                #exit()
    log_result(genotypes,epoch,logging)
